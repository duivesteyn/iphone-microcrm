...
NSInteger currencyisworking3 = 0;
...
+ (void) getInitialDataToDisplay:(NSString *)dbPath {
	...
	if (sqlite3_open([dbPath UTF8String], &database) == SQLITE_OK) {
		...
		const char *sql = "select coffeeID, coffeeName, price, companyName, leadDate, Notes, ClientID, ABRecordRef from Leads where section = 3";
		...
		if(sqlite3_prepare_v2(database, sql, -1, &selectstmt, NULL) == SQLITE_OK) {
			...
			while(sqlite3_step(selectstmt) == SQLITE_ROW) {
				...
				Invoices *coffeeObj3 = [[Invoices alloc] initWithPrimaryKey:primaryKey];
				coffeeObj3.coffeeName = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 1)];
				//de.002 added line
				coffeeObj3.price = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 2)];
				coffeeObj3.companyName = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 3)];
				coffeeObj3.LeadDate = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 4)];
				coffeeObj3.notes = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 5)];
				coffeeObj3.clientID = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 6)];
				coffeeObj3.personID = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 7)];							
				

				
				coffeeObj3.isDirty = NO;
				
				[appDelegate.coffeeArray3 addObject:coffeeObj3];
				[coffeeObj3 release];
			}
...
- (void) addInvoices {
	...
	sqlite3_bind_text(addStmt, 6, [@"3" UTF8String], -1, SQLITE_TRANSIENT);
	if ([clientID length] == 0) clientID = @" "; //(This line caused a clientID=0 bug in 1.0.0)
	sqlite3_bind_text(addStmt, 7, [clientID UTF8String], -1, SQLITE_TRANSIENT);
...
- (void) saveAllDataInvoices {
	section = @"3";
	NSLog(@"in saveAllDataInvoices in Invoices.m");
	
	if(isDirty) {
		
		if(updateStmt == nil) {
			const char *sql = "update Leads Set CoffeeName = ?, Price = ?, CompanyName = ?, LeadDate = ?, Notes = ?, Section = ?, ClientID = ?, ABRecordRef =? Where CoffeeID = ?";
		...
		sqlite3_bind_text(updateStmt, 6, [section UTF8String], -1, SQLITE_TRANSIENT);
		//if (clientID == nil) clientID = @"0";
		sqlite3_bind_text(updateStmt, 7, [clientID UTF8String], -1, SQLITE_TRANSIENT);		

		if (personID == nil) personID = @"0";
		sqlite3_bind_text(updateStmt, 8, [personID UTF8String], -1, SQLITE_TRANSIENT);	
		sqlite3_bind_int(updateStmt, 9, coffeeID);
