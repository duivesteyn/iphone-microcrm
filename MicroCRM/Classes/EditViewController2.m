...
- (void)viewWillAppear:(BOOL)animated {
	...
	if(keyOfTheFieldToEdit == @"coffeeName"){
		...
		//Set Icon
		imgView.image = [UIImage imageNamed:@"ico-job.png"];
		...
		self.title = @"Job Value";
		
		//txtField.hidden = NO;
		txtField.enabled = YES;
		txtField.placeholder = @"Job Value";
...
- (IBAction) save_Clicked:(id)sender {
	...
	//UIAlertView *baseAlert2 = [[UIAlertView alloc]  initWithTitle:@"Save_Clicked" message:keyOfTheFieldToEdit delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
	//[baseAlert2 show];
