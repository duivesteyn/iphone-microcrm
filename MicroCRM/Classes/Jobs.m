...
NSInteger currencyisworking2 = 0;
...
+ (void) getInitialDataToDisplay:(NSString *)dbPath {
	...
	if (sqlite3_open([dbPath UTF8String], &database) == SQLITE_OK) {
		...
		const char *sql = "select coffeeID, coffeeName, price, companyName, leadDate, Notes, ClientID, ABRecordRef from Leads where section = 2";
		...
		if(sqlite3_prepare_v2(database, sql, -1, &selectstmt, NULL) == SQLITE_OK) {
			...
			while(sqlite3_step(selectstmt) == SQLITE_ROW) {
				...
				Jobs *coffeeObj2 = [[Jobs alloc] initWithPrimaryKey:primaryKey];
				coffeeObj2.coffeeName = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 1)];
				//de.002 added lines
				coffeeObj2.price = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 2)];
				coffeeObj2.companyName = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 3)];
				coffeeObj2.LeadDate = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 4)];
				coffeeObj2.notes = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 5)];
				coffeeObj2.clientID = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 6)];				
				coffeeObj2.personID = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 7)];			
				

				
				coffeeObj2.isDirty = NO;
				
				[appDelegate.coffeeArray2 addObject:coffeeObj2];
				[coffeeObj2 release];
			}
...
- (void) addJobs {
	...
	sqlite3_bind_text(addStmt, 6, [@"2" UTF8String], -1, SQLITE_TRANSIENT);
...
- (void) saveAllDataJobs {
	section = @"2"; 
	NSLog(@"in saveAllDataJobs in Jobs.m");
